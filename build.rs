...
fn build_windows() {
    ...
    let platform_bits:u32 = platform_bits.parse().unwrap_or_else(|platform_bits|{
        panic!("Env {}='{}' is not valid u32","CARGO_CFG_TARGET_POINTER_WIDTH",platform_bits);
    });
    let cb_env_var = match platform_bits {
        64 => "ClientBridge64",
        32 => "ClientBridge",
        _  => panic!("Unexpected pointer width: {} (variable {})",
                     platform_bits,"CARGO_CFG_TARGET_POINTER_WIDTH"),
    };

    let cb_base_dir = env::var(cb_env_var).unwrap_or_else(|cb_env_var|{
        panic!("Env Variable '{}' not set. Ensure that you have ClientBridge/C++ installed.",cb_env_var);
    });
